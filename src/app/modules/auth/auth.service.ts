...
const googleLogin = async (idToken: string): Promise<ILoginResponse> => {
  const ticket = await googleClient.verifyIdToken({
    idToken,
    audience: config.googleAuthWebClientId,
  });
  ...
  if (!payload.email_verified) {
    throw new ApiError(
      ...
      'Email is not Verified! Please login with verified email.',
    );
  }
